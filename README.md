...
npx cypress open
```
This will open the Cypress Test Runner and allows you to choose which browser to run against, changing options and which testing scenarios to run.
...
## Tests Included
Under the tight deadline of 3 days, I've done a quick risk based analysis,what the core functionality of and e-commerce company is, and created the minimum E2E test that I think the web application shouldn't be released if any of them fail. I've assumed that the current state of the web application is the desired state, which is something I wouldn't do in a real life situation, as I would use the requirements instead (Please see the notes at the end of the README on existing and potential issues I've discovered in the web application).
...
- Positive: The product data is consistent between homepage and product details page, including product name, description and price
- Positive: The product data is consistent between product details and shopping cart pages, including product name and price
...
## Best Practices Used
### Page Object Model (POM)
When designing the testing framework, I've POM design pattern and spent the longer time designing the page objects (found under /pages). This effort investment pays off on the long run insuring:
...
Although the current web application is currently only in English, it's common that application support multiple languages in the future. The test suite is built ready to be extended for that in the future if needed. The strings used (e.g. checking for the correct alerts text) are not hardcoded in tests but rather in /fixtures/strings/en.json file. If in the future the application added support for another language interface - for example German - there would be no need to touch the existing tests. Rather make a copy of en.json to de.json, replace the English text with the German one, change `lang` in the configuration file /cypress.config.js to `de` and the tests will adapt to any new languages automatically, utilizing Cypress command I've added in /commands.js and how the tests were designed. That is also handy if the string changed in the current english string, since there's one place to updates.
...
- Some element selectors are not the best. In real scenario, I would talk to the developers to add better ones for those, eg. `id` or  `data-testid`. There are also elements with repeated `id` attribute (e.g. the left side categories links all have `id="itemc"` which make the html invalid). I've tried to find the best available options in those cases, without depending on strings (will break with localization) or fragile XPath. With the current test framework design, it's easy to update the selectors in one place on the top of the page object file.
